...
import { embedText as srcEmbedText } from '../src/embed';
import { embedText as mcpEmbedText } from '../mcp/src/embed';

// ── Parity tests ─────────────────────────────────────────────────────────────
// The gardener, src, and mcp embedText functions must have compatible signatures.
// This test verifies structural parity, not network behavior.

describe('embedText parity', () => {
  it('gardener embedText has the same parameter structure as src/embed.ts', () => {
    expect(typeof gardenerEmbedText).toBe('function');
    expect(typeof srcEmbedText).toBe('function');
    // Both accept (client, config, text) — 3 params
    expect(gardenerEmbedText.length).toBe(3);
    expect(srcEmbedText.length).toBe(3);
  });

...
**Title**: A claim or insight when one is present. A good title lets you scan a list and know what the fragment says without opening it. If the input doesn''t contain a claim, use a descriptive phrase. Never a topic label. Use the user''s vocabulary in titles, not academic equivalents or genre classifications. If the user said ''ongoing series like Community,'' don''t write ''ensemble comedies.'' If the user said ''agreed upon preference,'' don''t write ''contingent agreements.'' Preserve the user''s commitment level — if they said ''try'' or ''maybe'' or ''consider,'' the title should reflect that tentativeness, not upgrade it to a definitive action or inferred purpose.

**Body**: Use the user''s own words. Every sentence must be traceable to the input. Enough to land the idea, no more — shorter is better than padded, but completeness beats brevity when the idea requires it. Never compress, never add inferred meanings, never add a concluding sentence that summarizes what the user''s words already showed. Do not synthesize or interpret. The body is transcription, not synthesis. Preserve the user''s evaluative stance — both emotional reactions (''I liked,'' ''I''m missing,'' ''it felt like'') and assessments (''possibly the best,'' ''I prefer X over Y,'' ''better than,'' ''the right choice''). If the user judges, recommends, or ranks something, that judgment must appear in the body, not just the title. Do not restructure an opinion into a neutral description of features.');

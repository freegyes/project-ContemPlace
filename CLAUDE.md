...
Telegram → Telegram Worker → verify signature → check chat ID whitelist → /start → /undo → dedup check → return 200
                              ...
                              └→ capture: ctx.waitUntil():
                                   typing indicator
                                   → if photo: download from Telegram Bot API → upload to R2 → get public URL
...
```
src/
  index.ts           # Worker entry point (webhook handler, /undo command, photo download + R2 upload, Service Binding calls to MCP Worker, HTML reply formatting)
  config.ts          # Env var reading (Telegram + Supabase only — model/threshold config lives in MCP Worker)
  telegram.ts        # Telegram API helpers (sendMessage, sendChatAction, getFilePath, downloadTelegramFile)
  db.ts              # Supabase client + dedup only (createSupabaseClient, tryClaimUpdate)
  types.ts           # TypeScript interfaces (TelegramPhotoSize, CaptureServiceStub with options object, R2Bucket + R2_PUBLIC_URL in Env)
mcp/
...
.claude/
  ...
  commands/
    orchestrate.md      # Custom command: orchestrator mode — triage, parallel cmux workspaces + git worktrees, decision checkpoint protocol
    ...
    work-on-issue.md      # Custom command: full issue workflow — gather → hypothesis → review → plan → implement → verify → ship
...
- **Telegram Worker** — `TELEGRAM_BOT_TOKEN`, `TELEGRAM_WEBHOOK_SECRET`, `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY`, `ALLOWED_CHAT_IDS`. Plus `R2_PUBLIC_URL` in `wrangler.toml [vars]` and R2 bucket binding (`IMAGE_BUCKET`).
